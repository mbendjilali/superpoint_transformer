...
class SPT(nn.Module):
    ...
    def __init__(
        ...
        self.node_mlps = _build_mlps(
            node_mlp, num_down + self.nano, mlp_activation, mlp_norm, share_hf_mlps
        )

        h_edge_mlp = h_edge_mlp if needs_h_edge_hf else None
        self.h_edge_mlps = _build_mlps(
            h_edge_mlp, num_down + self.nano, mlp_activation, mlp_norm, share_hf_mlps
        )
        ...
        if num_up > 0:
            ...
            self.up_stages = nn.ModuleList(
                [
                    ...
                    for dim, num_blocks, in_mlp, out_mlp, mlp_drop, num_heads, ffn_ratio, residual_drop, attn_drop, drop_path, stage_k_rpe, stage_q_rpe in zip(
                        up_dim,
                        up_num_blocks,
                        up_in_mlp,
                        up_out_mlp,
        ...
        assert (
            self.num_up_stages > 0 or not self.output_stage_wise
        ), "At least one up stage is needed for output_stage_wise=True"

        assert (
            bool(self.down_stages) != bool(self.up_stages)
            ...
            for i_stage, (stage, node_mlp, h_edge_mlp, v_edge_mlp) in enum:
                ...
                if h_edge_mlp is not None:
                    ...
                    if edge_attr is not None:
                        nag[i_level].edge_attr = h_edge_mlp(edge_attr, batch=norm_index)
...
def _build_shared_rpe_encoders(rpe, num_stages, in_dim, out_dim, stages_share):
...
def _build_mlps(layers, num_stage, activation, norm, shared):
    ...
    return nn.ModuleList(
        [MLP(layers, activation=activation, norm=norm) for _ in range(num_stage)]
    )

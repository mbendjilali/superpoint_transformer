...
SEQUENCES = {k: list(set(osp.dirname(x) for x in v)) for k, v in WINDOWS.items()}
...
labels = [
    ...
    Label(
        "rectification border", 2, -1, IGNORE, "void", 0, False, True, True, (0, 0, 0)
    ),
    ...
    Label(
        "bridge", 15, 31, IGNORE, "construction", 2, False, True, True, (150, 100, 100)
    ),
    Label(
        "tunnel", 16, 32, IGNORE, "construction", 2, False, True, True, (150, 120, 90)
    ),
    ...
    Label(
        "vending machine", 40, 27, IGNORE, "object", 3, True, True, True, (128, 64, 0)
    ),
...
KITTIID2LABEL = {
    label.kittiId: label for label in labels
...
TRAINID2ID = np.asarray(
    [TRAINID2LABEL[c].id for c in range(KITTI360_NUM_CLASSES)] + [0]
)
